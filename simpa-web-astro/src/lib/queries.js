...
/**
 * Aktif hero slaytları, dile göre doğru alanlar seçilmiş halde. Boş alanlar null
 * kalır — render tarafı "tanımlıysa göster" mantığını uygular.
 */
...
export async function getProductOptionsList(db, lang) {
  const titleCol = lang === 'en' ? 'title_en' : 'title_tr';
  const { results } = await db
    .prepare(`SELECT prod_code, ${titleCol} as title FROM products WHERE ${titleCol} IS NOT NULL AND is_active = 1 ORDER BY prod_code`)
    .all();
  return results;
}
  ...
  const { results } = await db
    .prepare(
      `SELECT c.id, c.${nameCol} as name, c.slug, c.image_url,
       ...
       FROM categories c
       WHERE c.brand = 'Simpa' OR c.brand IS NULL
